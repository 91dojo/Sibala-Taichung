...
{
    public class SibalaComparer : IComparer<ISibala>
    {
        ...
        private static int CompareWhenSameType(ISibala x, ISibala y)
        {
            var comparers = new Dictionary<EnumOutputType, Func<ISibala, ISibala, int>>()
            {
                ...
                {EnumOutputType.NPoints, CompareWhenNormalPoints},
                {EnumOutputType.NoPoint, CompareWhenNoPoints},
            };
            return comparers[x.OutputType].Invoke(x, y);
        }

        private static int CompareWhenNoPoints(ISibala sibala1, ISibala sibala)
        {
            return 0;
        }

        private static int CompareWhenNormalPoints(ISibala x, ISibala y)
        {
            if (x.Point == y.Point)
            {
